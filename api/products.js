...
const serializeCoupon = ({ _id, ...coupon }) => ({ ...coupon, id: _id.toString() })

function generateCouponCode(prefix = 'MATE') {
...
function normalizeCouponDoc(body = {}) {
  ...
  return {
    code: normalizeCouponCode(body.code || generateCouponCode(body.prefix)),
...
export default async function handler(req, res) {
  ...
  // CUPONES: /api/products?resource=coupons
  if (resource === 'coupons') {
    try {
      const db = await getDb()
